...
//  System.out.println("Colors used are :"+nclrused);
  ...
  public static Image read(FileInputStream fs) {
    try {
      ...
      bh.read(fs);
      if (bh.nbitcount == 24)return (readMap24(fs, bh));
      if (bh.nbitcount == 32)return (readMap32(fs, bh));
      if (bh.nbitcount == 8)return (readMap8(fs, bh));
      fs.close();
    }
    catch (IOException e) {
// System.out.println("Caught exception in loadbitmap!");
  ...
  protected static Image readMap32(FileInputStream fs, BitmapHeader bh) throws
    ...
    image = Toolkit.getDefaultToolkit().createImage(new MemoryImageSource(bh.
        nwidth, bh.nheight, ndata, 0, bh.nwidth));
    fs.close();
    return (image);
  }
...
// No Palatte data for 24-bit format but scan lines are
// padded out to even 4-byte boundaries.
    int npad = 2;//(bh.nsizeimage / bh.nheight) - bh.nwidth * 3;
    int ndata[] = new int[bh.nheight * bh.nwidth];
  ...
  /**     *      * readMap8 internal routine to read the bytes in a 8 bit bitmap     *      *      *      * Arguments:     *      * fs - file stream     *      * bh - header struct     *      * Returns:     *      * Image Object, be sure to check for (Image)null !!!!     *      *      *       */
...
// Scan lines are still padded out to even 4-byte
    ...
    for (int j8 = 0; j8 < bh.nheight; j8++) {
      for (int i8 = 0; i8 < bh.nwidth; i8++) {
        ndata8[bh.nwidth * (bh.nheight - j8 - 1) +
    ...
    image = Toolkit.getDefaultToolkit().createImage(new MemoryImageSource(bh.
        nwidth, bh.nheight, ndata8, 0, bh.nwidth));
...
//      System.exit(0);
//    }
    FileInputStream in = new FileInputStream(System.getProperty("user.dir") + "\\zp.bmp");
