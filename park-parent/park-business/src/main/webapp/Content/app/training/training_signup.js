(function ($) {
    ...
    $("#class_id").on('change', function (e) {
        ...
        if ($(this).val() == "0") {
            ...
            $.post('/training/classInfo', {classId: $(this).val()}, function (res) {
                var data = res.data;
    ...
    $("#confirm_signup").on("click", function (e) {
        ...
        $.post('/training/signTrainsStudents', conditions, function (res) {
            $form.attr("submitting", "");

            if (res.code == 1) {
                //支付
                $.tipsSuccessAlert('报名成功！', function () {
                    location.assign('/training/students?classId=' + $("#class_id").val());
                });
            } else {
